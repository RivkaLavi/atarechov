...
;( function( $ ){
	...
	var jvbpd_common_func = function(){
		if( jvbpd_common_args   )
			this.param = jvbpd_common_args ;
		this.init();
	}
	jvbpd_common_func.prototype = {
		constructor : jvbpd_common_func,
		...
		init : function() {
			...
			; $( window )
				...
				.on( 'lava:favorite-not-user-loggedin', this.favorite_not_user_loggedin() )
				.on( 'lava:favorite-after', this.favorite_dashboard() )
		},
		...
		favorite_not_user_loggedin : function() {
			return function( event ) {
				$( '#login_panel' ).modal();
			}
		},

		favorite_dashboard : function() {
			return function( event, element, data ) {
				if( data.dashboard && ! data.is_saved ) {
		...
		init_main_affix : function(){
			...
			var
				...
				adminbar = $( '#wpadminbar' ),
				content_cotainer = $( "div#content-page-wrapper" );
		...
		login_via_google_oauth : function(){
			...
			return function() {
				...
				$.post(
					ajaxurl,
					{ action:'jvbpd_ajax_google_login_oauth' },
	...
	javo.common = new jvbpd_common_func;
	window.javo = javo;
})( jQuery );
	...
	var	jvbpd_single_top_nav	= {
		...
		, scroll:function(){
			...
			$('.javo-single-nav	 a').each(function(i,	k){
				...
				if(
					$tar.length &&
					$scrTop	>= ( $tar.offset().top - jvbpd_single_top_nav.offset	) &&
					$scrTop	< (	$tar.offset().top +	$tar.outerHeight(true) - jvbpd_single_top_nav.offset	 )
				){
		...
		, click:function(e){
			...
			$( 'html, body' ).animate({
				scrollTop: ( $tar.offset().top - jvbpd_single_top_nav.offset	+ 4	)  + 'px'
			}, 500 );
		}
	};

	jvbpd_single_top_nav.init();
	window.jvbpd_single_effect =	{
		...
		,
		scroll:	function(){
			var	$object	= window.jvbpd_single_effect;
