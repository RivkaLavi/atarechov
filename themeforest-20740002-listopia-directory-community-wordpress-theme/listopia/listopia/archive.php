...
<?php endif; ?>
	...
	<h3 class="archive-title">
		<?php
		if ( is_day() ) :
			printf( wp_kses( __( 'Daily Archives: %s', 'listopia' ), jvbpd_allow_tags()), '<span>' . get_the_date() . '</span>' );
		elseif ( is_month() ) :
			printf( wp_kses( __( 'Monthly Archives: %s', 'listopia' ), jvbpd_allow_tags()), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'listopia' ) ) . '</span>' );
		elseif ( is_year() ) :
			printf( wp_kses( __( 'Yearly Archives: %s', 'listopia' ), jvbpd_allow_tags()), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'listopia' ) ) . '</span>' );
		elseif( is_author() ) :
			printf( "%s <span>%s</span><i></i>",
				esc_html( get_userdata( get_queried_object_id() )->display_name ),
				esc_html__( "Author", 'listopia' )
			);
		else:
			printf(
				wp_kses(
					__('%s <span>Archive</span>', 'listopia' ),
					Array( 'span' => Array() )
				),
