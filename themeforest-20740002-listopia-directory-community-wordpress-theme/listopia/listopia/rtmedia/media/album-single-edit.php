...
<div class="rtmedia-container rtmedia-single-container rtmedia-media-edit">
	<?php if ( rtmedia_is_global_album( $rtmedia_query->media_query['album_id'] ) ) { ?>
		<h2><?php esc_html_e( 'Edit Album : ', 'listopia' );
			echo esc_html( $rtmedia_media->media_title ); ?></h2>

		<div class="rtmedia-edit-media-tabs rtmedia-editor-main">
			<ul class="rtm-tabs clearfix">
				<li class="active"><a href="#details-tab"><i
							class='dashicons dashicons-edit rtmicon'></i><?php esc_html_e( 'Details', 'listopia' ); ?>
					</a></li>
				<?php if ( ! is_rtmedia_group_album() ) { ?>
					<li class=""><a href="#manage-media-tab"><i
								class='dashicons dashicons-list-view rtmicon'></i><?php esc_html_e( 'Manage Media', 'listopia' ); ?>
						</a></li>
				<?php } ?>
				...
				<div class="content active" id="details-tab">
					<form method="post" class="rtm-form">
						...
						<div class="rtmedia-edit-title rtm-field-wrap">
							<label for="media_title"><?php esc_html_e( 'Title : ', 'listopia' ); ?></label>
							<?php rtmedia_title_input(); ?>
						</div>

						<div class="rtmedia-editor-description rtm-field-wrap">
							<label for='description'><?php esc_html_e( 'Description: ', 'listopia' ) ?></label>
							<?php
						...
						<div>
							<input type="submit" name="submit" class='rtmedia-save-album'
							       value="<?php esc_attr_e( 'Save Changes', 'listopia' ); ?>"/>
							...
							<form class="rtmedia-album-edit rtmedia-bulk-actions" method="post"
								...
								<p>
									<span><input type="checkbox" name="rtm-select-all" class="select-all"
									             title="<?php esc_attr_e( 'Select All Visible', 'listopia' ); ?>"/></span>
									<button class="button rtmedia-move" type='button'
									        title='<?php esc_attr_e( 'Move Selected media to another album.', 'listopia' ); ?>'><?php esc_html_e( 'Move', 'listopia' ); ?></button>
									...
									<button type="button" name="delete-selected" class="button rtmedia-delete-selected"
									        title='<?php esc_attr_e( 'Delete Selected media from the album.', 'listopia' ); ?>'><?php esc_html_e( 'Delete', 'listopia' ); ?></button>
								</p>

								<p class="rtmedia-move-container">
									<?php $global_albums = rtmedia_get_site_option( 'rtmedia-global-albums' ); ?>
									<span><?php esc_html_e( 'Move selected media to the album : ', 'listopia' ); ?></span>
									...
									<input type="button" class="rtmedia-move-selected" name="move-selected"
									       value="<?php esc_attr_e( 'Move Selected', 'listopia' ); ?>"/>
								</p>
								...
								<a id="rtMedia-galary-prev" style='<?php echo esc_attr( $display ); ?>'
								   href="<?php echo esc_url( rtmedia_pagination_prev_link() ); ?>"><?php esc_html_e( 'Prev', 'listopia' ); ?></a>
								...
								<a id="rtMedia-galary-next" style="<?php echo esc_attr( $display ); ?>"
								   href="<?php echo esc_url( rtmedia_pagination_next_link() ); ?>"><?php esc_html_e( 'Next', 'listopia' ); ?></a>
							</form>
						<?php } else { ?>
							<p><?php esc_html_e( 'The album is empty.', 'listopia' ); ?></p>
						<?php } ?>
					</div>
				<?php } ?>
	...
	<?php } else { ?>
		<p><?php esc_html_e( 'Sorry !! You can not edit this album.', 'listopia' ); ?></p>
	<?php } ?>
</div>
