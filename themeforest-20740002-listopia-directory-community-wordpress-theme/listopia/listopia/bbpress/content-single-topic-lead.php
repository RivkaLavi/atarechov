	...
	<div id="bbp-topic-<?php bbp_topic_id(); ?>-lead" class="bbp-lead-topic">
		<div id="post-<?php bbp_topic_id(); ?>" <?php bbp_topic_class(); ?>>
			...
			if( function_exists( 'bbp_get_topic_reply_count' ) ) {
				$arrThisTopicMeta[] = sprintf( "<i class='%1\$s'></i> " . esc_html__( "%2\$s Replies", 'listopia' ), 'jvbpd-icon3-comment2', bbp_get_topic_reply_count() );
			}
			if( function_exists( 'bbp_get_topic_voice_count' ) ) {
				$arrThisTopicMeta[] = sprintf( "<i class='%1\$s'></i> " . esc_html__( "%2\$s Voices", 'listopia' ), 'jvbpd-icon3-volume2', bbp_get_topic_voice_count() );
			}
			...
			if( function_exists( 'pvc_get_post_views' ) ) {
				$arrThisTopicMeta[] = sprintf( "<i class='%1\$s'></i> " . esc_html__( "%2\$s Views", 'listopia' ), 'jvbpd-icon3-pie-chart', pvc_get_post_views() );
			} ?>
