		...
		<form id="new-post" name="new-post" method="post" action="<?php bbp_forum_permalink(); ?>">
			...
			<fieldset class="bbp-form">
				<legend>
					...
					<?php
						if ( bbp_is_forum_edit() ) :
							printf( esc_html__( 'Now Editing &ldquo;%s&rdquo;', 'listopia' ), bbp_get_forum_title() );
						else :
							bbp_is_single_forum()
								? printf( esc_html__( 'Create New Forum in &ldquo;%s&rdquo;', 'listopia' ), bbp_get_forum_title() )
								: esc_html_e( 'Create New Forum', 'listopia' );
						endif;
					?>
				...
				<div>
					...
					<p>
						<label for="bbp_forum_title"><?php printf( esc_html__( 'Forum Name (Maximum Length: %d):', 'listopia' ), bbp_get_title_max_length() ); ?></label><br />
					...
					if ( function_exists( 'bbp_allow_forum_mods' ) && bbp_allow_forum_mods() && current_user_can( 'assign_moderators' ) ) : ?>
						...
						<p>
							<label for="bbp_moderators"><?php esc_html_e( 'Forum Moderators:', 'listopia' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_type"><?php esc_html_e( 'Forum Type:', 'listopia' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_status"><?php esc_html_e( 'Status:', 'listopia' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_visibility"><?php esc_html_e( 'Visibility:', 'listopia' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_parent_id"><?php esc_html_e( 'Parent Forum:', 'listopia' ); ?></label><br />

						<?php
							bbp_dropdown( array(
								'select_id' => 'bbp_forum_parent_id',
								'show_none' => esc_html__( '&mdash; No parent &mdash;', 'listopia' ),
					...
					<div class="bbp-submit-wrapper">
						...
						<button type="submit" id="bbp_forum_submit" name="bbp_forum_submit" class="button submit"><?php esc_html_e( 'Submit', 'listopia' ); ?></button>
...
<?php elseif ( bbp_is_forum_closed() ) : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<ul>
				<li><?php printf( esc_html__( 'The forum &#8216;%s&#8217; is closed to new content.', 'listopia' ), bbp_get_forum_title() ); ?></li>
			</ul>
		</div>
	</div>
		...
		<div class="bbp-template-notice">
			<ul>
				<li><?php is_user_logged_in()
					? esc_html_e( 'You cannot create new forums.',               'listopia' )
					: esc_html_e( 'You must be logged in to create new forums.', 'listopia' );
				?></li>
			</ul>
		</div>
	</div>
