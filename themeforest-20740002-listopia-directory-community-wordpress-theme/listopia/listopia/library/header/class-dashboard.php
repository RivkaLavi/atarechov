...
class jvbpd_dashboard {
	...
	public function getDashboardTItle() {
		...
		$strTitleFormat = esc_html__( '%1$s Profile', 'listopia' );
		if( $this->checkPermission( 'memberOnly' ) ) {
			$strTitleFormat = esc_html__( '%1$s Dashboard', 'listopia' );
		}
	...
	public function getBreadCrumbs() {
		...
		$arrBreadCrumbs[] = esc_html__( "Dashboard", 'listopia' );
	...
	public function dashboardTitle( $title, $sep=' | ' ) {
		$arrOutput = Array( $this->getDashboardTItle(), esc_html__( "My Dashboard", 'listopia' ), get_bloginfo( 'name' ) );
