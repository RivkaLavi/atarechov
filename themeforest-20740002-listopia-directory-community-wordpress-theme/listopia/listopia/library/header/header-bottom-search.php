...
<div id="wrapper">
	...
	<nav <?php jvbpd_layout()->getNavClass(); ?>>
		...
		<?php if( apply_filters( 'jvbpd_middle_header_display', false ) ) : ?>
			<div class="middle-header">
				<div class="container">
					...
					<?php
					$headerSearchType = false;
					if( $headerSearchType && shortcode_exists( 'jvbpd_search1' ) ) {
						echo '<div style="max-width:600px; display:inline-block; vertical-align:middle; position:relative; line-height:1;">';
						echo do_shortcode( sprintf( '[jvbpd_search1 columns="2" keyword_auto="disable" column1="ajax_search" column2="listing_location_with_google_search" amenities_field="disable" break_submit=true query_requester="%1$s"]', jvbpd_tso()->get( 'search_sesult_page' ) ) );
						echo '</div>';
					}
					if( !$headerSearchType && shortcode_exists( 'lava_ajax_search_form' ) ) {
		...
		<div class="jvbpd_header_bottom_wrap">
			<div class="container">
				<?php do_action( 'jvbpd_top_nav_menu_center' ); ?>
			</div>
		</div>
	</nav>
	<?php endif; ?>