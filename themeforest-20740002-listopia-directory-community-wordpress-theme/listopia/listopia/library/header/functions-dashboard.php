	...
	$arrMyMenus = apply_filters(
		...
		Array(
			'' => Array(
				...
				'label' => esc_html__("Dashboard", 'listopia' ),
				'visible' => 'all',
			),
			...
			JVBPD_PROFILE_SLUG => Array(
				...
				'label' => esc_html__("Edit Profile", 'listopia' ),
				'visible' => 'member',
			),
		)
	);
...
function jvbpd_dashboard_change_pw() {
	...
	try{
		...
		if( ! wp_verify_nonce( $_POST[ 'jvbpd_dashboard_changepw_nonce' ], 'security' ) ) {
			throw new Exception( esc_html__( "Password does not match the confirm password.", 'listopia' ) );
		}
		...
		if( $new_password == '' || $new_password_confirm == '' )
			throw new Exception( esc_html__( "Please check your password or password confirm.", 'listopia' ) );
	...
	return esc_html__( "It has been successfully changed,", 'listopia' );
}