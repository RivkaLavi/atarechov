...
if( class_exists( 'Lynk_Core' ) && ! property_exists( 'Lynk_Core', 'prefix' ) ) {
	...
	wp_die(
		esc_html__( "The old version of Core plugin has been deactivated due to compatibility. Please update Core plugin to the newest version.", 'listopia' ) .
		sprintf( '<p><a href="%1$s" target="_self" title="%2$s">%2$s</a></p>', admin_url( 'plugins.php' ), esc_html__( "Go to plugin update", 'listopia' ) )
	);
}
...
if( class_exists( 'Jvbpd_Core_AdminHelper' ) ) {
	class jvbpd_admin_helper extends Jvbpd_Core_AdminHelper {
		...
		public function createHelperMenu() {
			...
			$this->addSub( Array(
				'slug' => 'status',
				'name' => esc_html__( "Status", 'listopia' ),
				'func' => array( $this, 'helper_staus' ),
			) );

			$this->addSub( Array(
				'slug' => 'plugins',
				'name' => esc_html__( "Plugins", 'listopia' ),
				'func' => array( $this, 'helper_plugins' ),
			) );

			if( class_exists( 'jvbpd_theme_settings' ) ) {

				$this->addSub( Array(
					'slug' => 'settings',
					'name' => esc_html__( "Theme Settings", 'listopia' ),
					'func' => array( jvbpd_theme_settings::$instance, 'settings_page_initialize' ),
				) );
			}
		}
		...
		public function get_template( $template_name )  {
			...
			do_action( 'jvbpd_admin_helper_' . $template_name . ' _header' );
			if( file_exists( $strFileName ) )
				require_once $strFileName;

			do_action( 'jvbpd_admin_helper_' . $template_name . '_footer' );
		...
		public function helper_page_header()
		{
			...
			printf( '<a href="%1$s" target="_blank" style="display:block;margin:10px;"><img src="%2$s"></a>',
				esc_url( 'javothemes.com/support/free-installation/' ),
				esc_url( JVBPD_THEME_DIR . '/assets/images/theme-settings-banner.png' )
			);
					...
					<h2 class="nav-tab-wrapper">
						...
						<?php
							foreach ($td_welcome_menu_items as $td_welcome_menu_item) {
								?>
									<a href="<?php echo esc_url( add_query_arg( Array( 'page' => $td_welcome_menu_item[2] ), admin_url( 'admin.php' ) ) ); ?>" class="nav-tab <?php if(isset($_GET['page']) and $_GET['page'] == $td_welcome_menu_item[2]) { echo sanitize_html_class( 'nav-tab-active' ); }?> "><?php  echo esc_html( $td_welcome_menu_item[0] ); ?></a>
								<?php
							}
						?>
					</h2>
		...
		public function tgmpa_return_link( $link='' ) {
			if(
				...
				$link = sprintf(
					'<a href="%1$s">%2$s</a>',
					esc_url( $url ),
					esc_html__( 'Return Javo Core Plugin Page', 'listopia' )
				);
			}
			return $link;
		}
