...
class jvbpd_theme_admin {
	...
	public function memoryCheck() {
		...
		$strDetailLink = sprintf(
			...
			esc_html__( "How to increase", 'listopia' )
		);
		...
		$this->addNotice(
			...
			'warning-lg',
			sprintf(
				__( '<strong>Javo Themes</strong><br>%5$sPHP Memory: %1$s( Recommended: %2$s ), WordPress Memory: %3$s( Recommended: %4$s )%6$s', 'listopia' ),
				$intPHP_Memory,
				$intPHP_Memory_Allow,
				$intWP_Memory,
				$intWP_Memory_Allow,
				esc_html__( "Your max uploading size is less than 128MB. It may cause some problems. Please increase your uploading size.", 'listopia' ) . '<br>',
				$strDetailLink
			)
		);
	}
	...
	public function add_featured_term( $tag )
	{
		...
		<div class="form-field jv-uploader-wrap">
			<label for="jvbpd_category_featured"><?php esc_html_e( 'Category Featured Image', 'listopia' );?></label>
			<img style="max-width:80%;"><br>
			<input type="text" name="jvbpd_category_featured" id="jvbpd_category_featured" data-id class="hidden">
			<button type="button" class="button button-primary upload" data-featured-field="[name='jvbpd_category_featured']"><?php esc_html_e('Change', 'listopia' );?></button>
			<button type="button" class="button remove"><?php esc_html_e( 'Remove' , 'listopia' );?></button>
		</div>
		<?php
	}
	...
	public function edit_featured_term($tag, $taxonomy) {
		...
		<tr class="form-field">
			...
			<td class="jv-uploader-wrap">
				<img src="<?php echo esc_url( $jvbpd_featured_src );?>" style="max-width:80%;"><br>
	...
	public function featured_term_columns($category_columns) {
		$new_columns		= array(
			...
			, 'name'		=> esc_html__('Name', 'listopia' )
			, 'description'	=> esc_html__('Description', 'listopia' )
			, 'featured'	=> esc_html__('Featured Preview', 'listopia' )
			, 'slug'		=> esc_html__('Slug', 'listopia' )
			, 'posts'		=> esc_html__('Items', 'listopia' )
		);
		return $new_columns;
	}
	...
	public function apparence_menu_append_option( $item, $item_id=0 ) {
		$menus = Array();
		$isJvbpdMenu = in_array( 'jvbpd-menu', $item->classes );
		if( $isJvbpdMenu ) {
			if( in_array( 'jvbpd-my_menu-nav', $item->classes ) ) {
				$menus[] = 'background';
			}
		}else{
			if( 0 === intVal( $item->menu_item_parent ) ) {
				$menus[] = 'wide-switcher';
				$menus[] = 'block-shortcode';
				$menus[] = 'background';
			}
			$menus[] = 'scrollspy';
			$menus[] = 'icon';
