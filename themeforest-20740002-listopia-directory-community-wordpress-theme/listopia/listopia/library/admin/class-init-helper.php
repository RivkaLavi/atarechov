...
class jvbpd_init_helper {
	...
	public function helper_content() {
		...
		$default_steps = array(
			'introduction' => array(
				'name'    => esc_html__( 'Introduction', 'listopia' ),
			...
			'addons' => array(
				'name'    => esc_html__( 'Addons', 'listopia' ),
				'view'    => array( $this, 'addons_page' ),
				'handler' => '',
				'priority' => 20,
			),
			'status' => array(
				'name'    => esc_html__( 'Server Status', 'listopia' ),
				'view'    => array( $this, 'status_page' ),
				'handler' => array( $this, 'wc_setup_location_save' ),
				'priority' => 30,
			),
			'demo_import' => array(
				'name'    => esc_html__( 'Demo import', 'listopia' ),
				'view'    => array( $this, 'import_page' ),
				'handler' => '',
				'priority' => 40,
			),
			'next_steps' => array(
				'name'    => esc_html__( 'Ready!', 'listopia' ),
		...
		if ( ! current_user_can( 'install_themes' ) || ! current_user_can( 'switch_themes' ) || is_multisite() || current_theme_supports( 'listopia' ) ) {
		...
		wp_localize_script(
			...
			Array(
				...
				'verify_text' => ' ' . esc_html__( "Verifying...", 'listopia' ),
			)
		);
	...
	public function ajax_active_plugins( $slug, $plugins=Array() ) {
		if ( ! $slug ) {
			wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'listopia' ) ) );
		}
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] == $slug ) {
				$json = array(
					...
					'message'       => esc_html__( 'Activating Plugin', 'listopia' ),
				);
				break;
			}
		}
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] == $slug ) {
				$json = array(
					...
					'message'       => esc_html__( 'Updating Plugin', 'listopia' ),
				);
				break;
			}
		}
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] == $slug ) {
				$json = array(
					...
					'message'       => esc_html__( 'Installing Plugin', 'listopia' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			...
			wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'listopia' ) ) );
		}
		exit;
	...
	public function setup_wizard_header() {
		...
		<head>
			...
			<?php printf( '<%1$s>%2$s</%1$s>', 'title', esc_html__( 'Listopia &rsaquo; Setup Wizard', 'listopia' ) ); ?>
			<?php
		...
		<body class="jvbpd-wizard wp-core-ui">
			<h1 id="jvbpd-logo"><img src="<?php echo esc_url( JVBPD_IMG_DIR . '/jv-logo2.png' ); ?>" alt="logo2" /></h1>
		<?php
	}
	...
	public function setup_wizard_footer() {
		?>
			<?php if ( 'next_steps' === $this->step ) : ?>
				<a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'listopia' ); ?></a>
			<?php endif; ?>
			</body>
		</html>
		<?php
	...
	public function addons_page() {
		jvbpd_admin()->load_template( Array( 'name' => 'addons', 'sub' => 'helper' ), Array( 'helper' => &$this ) );
	}
