...
class jvbpd_post_meta_func {
	...
	public static function post_meta_box_init() {
		$jvbpd_meta_boxes = Array(
			...
			// ===============================================================
			'jvbpd_postFormat_link'			=> Array( 'postFormat_link_func'			, 'post'		, 'normal'	, 'high' , esc_html__("Link Format Options", 'listopia' ) )
			, 'jvbpd_postFormat_quote'		=> Array( 'postFormat_quote_func'		, 'post'		, 'normal'	, 'high' , esc_html__("Quote Format Options", 'listopia' ) )
			, 'jvbpd_postFormat_video'		=> Array( 'postFormat_video_func'		, 'post'		, 'normal'	, 'high' , esc_html__("Video Format Options", 'listopia' ) )
			, 'jvbpd_postFormat_audio'		=> Array( 'postFormat_audio_func'		, 'post'		, 'normal'	, 'high' , esc_html__("Audio Format Options", 'listopia' ) )
		);

		$jvbpd_meta_boxes_post_and_page = Array(
			// Post Meta ID.							MetaBox Callback						Position	Level		Description
			// ================================================================
			'jvbpd_page_settings'	=> Array( 'jvbpd_page_settings_box'	, null , 'normal'		, 'high', esc_html__( "Page Settings", 'listopia' ) )
		);
			...
			Array(
				'link_title'	=>
					Array(
						'element'	=> 'input'
						, 'type'	=> 'text'
						, 'label'	=> esc_html__( "Link", 'listopia' )
					)
			)
		);
	}
			...
			Array(
				'quote_format'	=>
					Array(
						'element'	=> 'input'
						, 'type'	=> 'text'
						, 'label'	=> esc_html__( "Quote", 'listopia' )
					)
			)
		);
	}
		...
		self::postFormat_field(
			Array(
				'video_format_choose' =>
					Array(
						'element'	=> 'select'
						, 'label'	=> esc_html__( "Choose Video Type", 'listopia' )
						, 'option'	=> Array(
							esc_html__( "Select Portal", 'listopia' )	=> ''
							, esc_html__( "Youtube", 'listopia' )		=> 'youtube'
							, esc_html__( "Vimeo", 'listopia' )			=> 'vimeo'
						)
					)
				, 'video_format_link'	=>
					Array(
						'element'	=> 'input'
						, 'type'	=> 'text'
						, 'label'	=> esc_html__( "Video ID", 'listopia' )
					)
			)
		);
	}
			...
			Array(
				'audio_link'		=>
					Array(
						'element'	=> 'input'
						, 'type'	=> 'text'
						, 'label'	=> esc_html__( "Audio Link", 'listopia' )
					)
			)
		);
	}
		...
		if( $name == 'admin' ) {
			wp_localize_script(
				...
				Array(
					...
					'strAddressFindFailed' => esc_html__( "Sorry, find address failed", 'listopia' ),
