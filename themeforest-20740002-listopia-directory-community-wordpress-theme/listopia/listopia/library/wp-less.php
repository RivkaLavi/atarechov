...
if( file_exists( JVBPD_FUC_DIR . '/class-lessinc.php' ) )
	require_once JVBPD_FUC_DIR . '/class-lessinc.php';
...
if ( ! class_exists( 'wp_less' ) ) {
	...
	class wp_less {
		...
		public function parse_stylesheet( $src, $handle ) {
			...
			if(wp_get_theme()->get('Template')=='')
				$this->vars['themeurl'] = '~"' . get_stylesheet_directory_uri() . '"';
			else
				$this->vars['themeurl'] = '~"' . home_url('/').'wp-content/themes/'.wp_get_theme()->get('Template') . '"';
			$this->vars['lessurl']  = '~"' . dirname( $src ) . '"';
			$this->vars['type'] = jvbpd_layout()->getThemeType();
		...
		public function save_parsed_css( $css_path, $file_contents ) {
			...
			$prefix = 'file_put_';
			call_user_func_array( $prefix . 'contents', Array( $css_path, $file_contents ) );
		}
