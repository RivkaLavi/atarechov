...
class jvbpd_Recent_Posts_widget extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			'description' => esc_html__( 'Recent posts with thumbnails widget.', 'listopia' ),
		);
		parent::__construct( 'jvbpd_recent_posts', esc_html__('[JAVO] Recent posts','listopia' ), $widget_ops );
	}
	...
	public function widget( $args, $instance ) {
		...
		<div class="widget_posts_wrap type-<?php echo sanitize_html_class( $jvbpd_this_post_type ); ?>">
			...
			if( $jvbpd_this_posts->have_posts() )
			{
				...
				esc_html_e('Not Found Posts.', 'listopia' );
			} ?>
	...
	function form( $instance ) {
		...
		foreach(
			Array(
				'title' => Array( 'label' => esc_html__( "Title", 'listopia' ), 'default' => '', ),
				'excerpt_length' => Array( 'label' => esc_html__( "Excerpt Length", 'listopia' ), 'default' => '20', ),
				'category' => Array(
					'label' => esc_html__( "Category", 'listopia' ),
					'desc' => esc_html__( "If there is no value, all posts are shown. If there is a value, posts included in the corresponding category are displayed. (Multiple values separated by a comma(,))", 'listopia' ),
					'default' => '',
				),
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'post_count' ) ); ?>"><?php esc_html_e( 'Limit:', 'listopia' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'post_type' ) ); ?>"><?php esc_html_e( 'Choose the Post Type: ' , 'listopia' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'featured_item' ) ); ?>">
				...
				<?php esc_html_e( 'Show only featured items: ' , 'listopia' ); ?>
			</label>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'describe_type' ) ); ?>"><?php esc_html_e( 'Description Type: ' , 'listopia' ); ?></label>
			<select class="widefat" id="<?php echo $this->get_field_id( 'describe_type' ); ?>" name="<?php echo $this->get_field_name( 'describe_type' ); ?>">
				<?php
				$arrDescribeType = apply_filters( 'jvbpd_recent_posts_widget_describe_type_options', Array( '' => esc_html__( 'Post Excerpt', 'listopia' ) ) );
