...
class jvbpd_enqueue_func
{
	...
	public function is_wpless_active() {
		return jvbpd_tso()->get( 'wp_less' ) === 'enable';
	}

	public function loadFiles() {
		// WP-LESS Enable for dev
		if( $this->is_wpless_active() ) {
			// This is an option for dev if you need.
	...
	public function backend_enqueue() {
		if( ! is_admin() )
			return;
		add_action( 'elementor/editor/after_enqueue_styles', Array( $this, 'elementor_enqueues_styles') );
		add_action( 'elementor/editor/after_enqueue_scripts', Array( $this, 'elementor_enqueues_scripts') );
		add_action('admin_enqueue_scripts', Array( __class__, 'admin_enqueue_callback' ), 1 );
	...
	public static function enqueue_scripts() {
		$jvbpd_register_scripts = Array(
			...
			'theia-sticky-sidebar.js' => 'theia-sticky-sidebar',
			...
			/* Masonry effect*/
			'masonry-custom/modernizr.custom.js' => 'modernizr-custom',
			'masonry-custom/classie.js' => 'classie',
			'masonry-custom/AnimOnScroll.js' => 'animonscroll',
		...
		*/
		...
		wp_localize_script(
			...
			Array(
				...
				'errUserName' => esc_html__( 'usernames with spaces should not be allowed.', 'listopia' ),
				'errDuplicateUser' => esc_html__( 'User Register failed. Please check duplicate email or Username.', 'listopia' ),
				'errNotAgree' => esc_html__( 'You need to read and agree the terms and conditions to register.', 'listopia' ),
				'strJoinComplete' => esc_html__( 'Register Complete', 'listopia' ),
			)
		);
		...
		$jvbpd_general_styles = Array(
			...
			'ows-carousel-style' => Array( 'file' => 'owl.carousel.min.css' ),
		);
	...
	public function elementor_enqueues_styles() {
		wp_enqueue_style( 'ows-carousel-style', JVBPD_THEME_DIR . '/assets/css/owl.carousel.min.css' );
	}

	public function elementor_enqueues_scripts() {
		wp_enqueue_script( 'owl-carousel', JVBPD_THEME_DIR . '/assets/js/owl.carousel.min.js', array( 'jquery' ), false, false );
		wp_enqueue_script( 'flexmenu', JVBPD_THEME_DIR . '/assets/js/flexmenu.min.js', array( 'jquery' ), false, true );
		wp_enqueue_script( 'jquery-javo-ajaxshortcode', JVBPD_THEME_DIR . '/assets/js/jquery.javo_ajaxShortcode.js', array( 'jquery' ), false, true );
	}
	...
	public static function admin_enqueue_callback() {
		self::$instance->loadIcons();
	...
	public function get_custom_fonts() {
			...
			if( $strFontFamily = jvbpd_tso()->get( $tag, false ) ) {
				...
				* into your own language. */
				if(
					...
					'off' === esc_html_x( 'on', 'Open Sans font: on or off', 'listopia' )
				) continue;
				...
				* into your own language. */
				if(
					...
					'off' === esc_html_x( 'on', 'Raleway font: on or off', 'listopia' )
				) continue;
				...
				* into your own language. */
				if(
					...
					'off' === esc_html_x( 'on', 'Roboto Condensed font: on or off', 'listopia' )
				) continue;
				...
				* into your own language. */
				if(
					...
					'off' === esc_html_x( 'on', 'Montserrat font: on or off', 'listopia' )
				) continue;
		...
		//  Basic Font
		$arrLoadFonts[] = 'Poppins:400,500,300,600,700';
	...
	public function admin_custom_style_apply_func( &$rows=Array() ) {
		...
		$rows[] = sprintf( '#wp%s{ position:fixed !important; }', 'adminbar' );
	...
	public static function post_enqueue_scripts_callback() {
		...
		if( "full" == $jvbpd_query->get("header_fullwidth", jvbpd_tso()->header->get( 'header_fullwidth', 'full' ) ) ){
		...
		}else if( "fixed-right" == $jvbpd_query->get("header_fullwidth", jvbpd_tso()->header->get( 'header_fullwidth', 'full' ) ) ){
		...
		if( false !== ( $hex = $jvbpd_query->get("mobile_header_bg", jvbpd_tso()->header->get( 'mobile_header_bg', false ) ) ) )
		{
			...
			$jvbpd_rgb = apply_filters( 'jvbpd_rgb', substr( $hex, 1) );
			$jvbpd_css_one_row[] = "html body.mobile .nav-top.navbar-static-top{ background-color:rgba( {$jvbpd_rgb['r']}, {$jvbpd_rgb['g']}, {$jvbpd_rgb['b']}, {$opacity}) !important; }";
		...
		// Mobile Header Skin
		{
			switch( $jvbpd_query->get("mobile_header_skin", jvbpd_tso()->header->get( 'mobile_header_skin', false ) ) )
			{
				...
				default:
				case "dark":
					$jvbpd_css_one_row[] = "html body.mobile #wrapper .navbar-header .navbar-toggle span,";
		...
		if( $color = $jvbpd_tso->get( 'footer_middle_background_color' , false ) )
			$jvbpd_css_one_row[] = "html body > #wrapper > div.footer-background-wrap{ background-color:{$color}; }";
	...
	public function defer_scripts( $url ) {
		...
		/*
		if ( strpos( $url, 'underscore' ) ) return $url;
		if ( strpos( $url, 'backbone' ) ) return $url;
