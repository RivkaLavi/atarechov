...
if ( ! function_exists( 'jvbpd_nav_menus' ) ) :
	...
	function jvbpd_nav_menus() {
		$coreSidebars = Array(
			'my_menu_nav' => esc_html__( 'My menu nav', 'listopia' ),
			'top_nav_menu_left' => esc_html__( 'Top Nav Menu - Left', 'listopia' ),
			'top_nav_menu_right' => esc_html__( 'Top Nav Menu - Right', 'listopia' ),
		);
		if( jvbpd_layout()->getThemeType() == 'jvd-lk' ) {
			$coreSidebars = wp_parse_args(
				Array(
					'topbar_left' => esc_html__( 'Top Bar - Left', 'listopia' ),
					'topbar_right' => esc_html__( 'Top Bar - Right', 'listopia' ),
					'sidebar_left' => esc_html__( 'Sidebar - Left', 'listopia' ),
					'sidebar_right' => esc_html__( 'Sidebar - Right', 'listopia' ),
				), $coreSidebars
			);
		}
		register_nav_menus( $coreSidebars );
	}
...
if( !function_exists( 'jvbpd_remove_rest_api' ) ) {
	function jvbpd_remove_rest_api() {
		remove_action ( 'wp_head', 'rest_output_link_wp_head', 10);
		remove_action ( 'wp_head', 'wp_oembed_add_discovery_links', 10);
	}
	add_action( 'init', 'jvbpd_remove_rest_api' );
}
...
function jvbpd_register_sidebars(){
	$jvbpd_sidebars = Array(
		Array(
			'name'			=> esc_html__( 'Default Sidebar', 'listopia' ),
			'id'			=> 'sidebar-1',
			'description'	=> esc_html__( 'Widgets in this area will be shown on the default pages.', 'listopia' ),
		...
		Array(
			'name'			=> esc_html__( 'Buddypress Sidebar', 'listopia' ),
			'id'			=> 'bp-sidebar',
			'description'	=> esc_html__( 'Widgets in this area will be shown on the default pages.', 'listopia' ),
		...
		Array(
			'name'			=> esc_html__( 'BBpress Sidebar', 'listopia' ),
			'id'			=> 'bb-sidebar',
			'description'	=> esc_html__( 'Widgets in this area will be shown on the default pages.', 'listopia' ),
		...
		Array(
			'name'			=> esc_html__( 'Woocommerce Sidebar', 'listopia' ),
			'id'			=> 'woo-sidebar',
			'description'	=> esc_html__( 'Widgets in this area will be shown on the default pages.', 'listopia' ),
			'before_widget'	=> '<div id="%1$s" class="widget widgets bp-sidebar clearfix %2$s">',
			'before_widget'	=> '<div class="widget widgets-wraps">',
	...
	for( $intCurWidget=1; $intCurWidget <= $intFooterColumns; $intCurWidget++ ) {
		$jvbpd_sidebars[] = Array(
			'name' => sprintf( esc_html__( 'Footer Sidebar%s', 'listopia' ), $intCurWidget ),
			'id' => 'sidebar-foot' . $intCurWidget,
			'description' => esc_html__( 'Widgets in this area will be shown on the footer side.', 'listopia' ),
			'before_widget' => '<div class="widget widgets-wraps">',
			'after_widget' => '</div>',
...
function jvbpd_setup() {
	 ...
	 * to change 'listopia' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'listopia', get_template_directory() . '/languages' );
...
function jvbpd_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 )
		$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'listopia' ), max( $paged, $page ) );
...
function jvbpd_content_nav() {
	...
	if ( $wp_query->max_num_pages > 1 ) : ?>
		<nav class="navigation jv-single-post-pager">
			<ul class="pager">
				<li class="next"><?php next_posts_link( wp_kses(__( 'Older posts <i class="fa fa-angle-left"></i>', 'listopia' ), jvbpd_allow_tags()) ); ?></li>
				<li class="previous"><?php previous_posts_link( wp_kses(__( '<i class="fa fa-angle-right"></i> Newer posts', 'listopia' ), jvbpd_allow_tags()) ); ?></li>
			</ul><!-- /.pager -->
		</nav>
	<?php endif;
}
...
function jvbpd_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class( 'jv-single-post-comment-item' ); ?> id="comment-<?php comment_ID(); ?>">
		<p><?php esc_html_e( 'Pingback:', 'listopia' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'listopia' ), '<span class="edit-link">', '</span>' ); ?></p>
	<?php
			break;
		default :
					...
					<div class="media-body">
						<div class="comment-author">
							...
							echo join( "\n", Array(
								...
								sprintf( '<small>%s at %s</small>', esc_html__('Posted at','listopia' ).' '.get_comment_date(), get_comment_time() ),
								sprintf( '<span>%s</span>',
									get_comment_reply_link(
										Array_merge( $args,
											Array(
												'reply_text'	=> esc_html__( 'Reply', 'listopia' ),
								...
								sprintf( "<a href=\"%s\">%s</a>", get_edit_comment_link( get_comment_ID() ), esc_html__( "Edit", 'listopia' ) ),
								'</div>',
							) ); ?>
						</div><!-- /.comment-author -->
						...
						<?php if ( '0' == $comment->comment_approved ) : ?>
							<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'listopia' ); ?></p>
						<?php endif; ?>
...
function jvbpd_entry_meta() {
	...
	$categories_list = get_the_category_list( esc_html__( ', ', 'listopia' ) );

	// Translators: used between list items, there is a space after the comma.
	$tag_list = get_the_tag_list( '', esc_html__( ', ', 'listopia' ) );
	...
	$author = sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
		...
		esc_attr( sprintf('%s %s', 'listopia' ), esc_html__('View all posts by', 'listopia' ), get_the_author() ),
		get_the_author()
	);

	// Translators: 1 is category, 2 is tag, 3 is the date and 4 is the author's name.
	if ( $tag_list ) {
		$utility_text = sprintf('%s : %%1$s  %s: %%2$s %s : %%3$s<span class="by-author"> %s %%4$s</span>.', esc_html__('Category', 'listopia' ), esc_html__('Tags', 'listopia' ), esc_html__('Date', 'listopia' ), esc_html__('by', 'listopia' ));
	} elseif ( $categories_list ) {
		$utility_text = sprintf('%s %%1$s %s : %%3$s<span class="by-author"> %s %%4$s</span>.', esc_html__('Category', 'listopia' ), esc_html__('Date', 'listopia' ), esc_html__('by', 'listopia' ));
	} else {
		$utility_text = sprintf('%s : %%3$s<span class="by-author"> %s %%4$s</span>.', esc_html__('Date', 'listopia' ), esc_html__('by', 'listopia' ));
	}
