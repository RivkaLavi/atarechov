...
class jvbpd_buddypress {
	...
	public function register_hooks() {
		...
		add_filter( 'bp_before_xprofile_cover_image_settings_parse_args', array( $this, 'jv_cover_image_css' ), 10, 1 );
		add_filter( 'bp_before_groups_cover_image_settings_parse_args', array( $this, 'jv_cover_image_css' ), 10, 1 );
	...
	public function admin_bar_in_header() {
		if ( !is_admin() ) {
			...
			add_action( 'jvbpd_after_body_tag', 'wp_admin_bar_render' );
		}
	}
	...
	public function bp_sidebar_position( $position='', $post_id=0 ) {
		if( function_exists( 'bp_is_user_messages' ) && bp_is_user_messages() ) {
			...
			foreach(
				Array(
					'member' => array( 'key' => 'bp_members_sidebar', 'default' => 'right', 'dir' => false ),
					'group' => array( 'key' => 'bp_group_sidebar', 'default' => 'right', 'dir' => true ),
					'activity' => array( 'key' => 'bp_activity_sidebar', 'default' => 'right', 'dir' => false ),
					'register' => array( 'key' => 'bp_register_sidebar', 'default' => 'right', 'dir' => false ),
					'profile' => array( 'key' => 'bp_profile_sidebar', 'default' => 'full', 'dir' => false ),
					'mypage' => array( 'key' => 'bp_mypage_sidebar', 'default' => 'full', 'dir' => false ),
				) as $component => $componentMeta
			) {
				if( $this->is_bp_page( $component, $componentMeta[ 'dir' ] ) ) {
					$position = jvbpd_tso()->get( $componentMeta[ 'key' ], $componentMeta[ 'default' ] );
				}
				if( $component == 'mypage' && $this->is_bp_page( Array( 'profile' ) ) && get_current_user_id() == bp_displayed_user_id() ) {
					$position = jvbpd_tso()->get( $componentMeta[ 'key' ], $componentMeta[ 'default' ] );
				}
			}
		}
	...
	public function bp_member_header_meta() {
		...
		$arrSocialMeta = Array(
			'facebook' => Array(
				'icon' => 'fa fa-facebook fa-stack-1x',
				'label' => esc_html__( "Facebook", 'listopia' ),
				'link' => $this->getBpMemberMeta( bp_displayed_user_id(), 'Facebook' ),
			),
			'twitter' => Array(
				'icon' => 'fa fa-twitter fa-stack-1x',
				'label' => esc_html__( "Twitter", 'listopia' ),
		...
		if( !empty( $arrSocialMeta ) ) {
			foreach( $arrSocialMeta as $strSocial => $arrMeta ) {
				...
				printf(
					'<a href="%1$s" target="_blank" title="%2$s"><span class="fa-stack"><i class="fa fa-circle-o fa-stack-2x"></i><i class="%3$s"></i></span></a>',
	...
	public function module_selector( $section='' ) {
		$targets = array( 'bp_members_', 'bp_group_' );
	...
	public function members_sidebar() {
		jvbpd_layout()->load_template( 'template-member-sidebar' );
	}
