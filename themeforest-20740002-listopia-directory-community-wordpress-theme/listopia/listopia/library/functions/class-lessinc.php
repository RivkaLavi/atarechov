...
class lessc {
	...
	const FILE_GET = 'file_get_';
	const FILE_PUT = 'file_put_';
	...
	protected function tryImport($importPath, $parentBlock, $out) {
		...
		$url = $this->compileValue($this->lib_echo($str));
		...
		$root = $parser->parse( call_user_func_array( self::FILE_GET . 'contents', Array( $realPath ) ) );
	...
	protected function compileProp($prop, $block, $out) {
		...
		case "directive":
			list(, $name, $value) = $prop;
			$out->lines[] = ( isset( $name ) ? "$name " : " " ) . $this->compileValue($this->reduce($value)).';';
			break;
		case "comment":
			$out->lines[] = $prop[1];
			break;
	...
	protected function lib_data_uri($value) {
		...
		if($fullpath && ($fsize = filesize($fullpath)) !== false) {
			...
			if($fsize/1024 < 32) {
				...
				$fileContent = call_user_func_array( self::FILE_GET . 'contents', Array( $fullpath ) );

				if(!is_null($mime)) // fallback if the mime type is still unknown
					$url = sprintf('data:%s;base64,%s', $mime, call_user_func_array( self::BASE64 . 'encode', $fileContent ) );
			}
		}
	...
	protected function lib_echo($arg) {
		switch ($arg[0]) {
			case "list":
				$items = $arg[2];
				if (isset($items[0])) {
					return $this->lib_echo($items[0]);
				}
	...
	protected function lib__sprintf($args) {
		...
		$template = $this->compileValue($this->lib_echo($string));
		...
		if (preg_match_all('/%[dsa]/', $template, $m)) {
			foreach ($m[0] as $match) {
				...
				$i++;
				$rep = $this->compileValue($this->lib_echo($val));
	...
	protected function lib_unit($arg) {
		if ($arg[0] == "list") {
			...
			return array("number", $this->assertNumber($number),
				$this->compileValue($this->lib_echo($newUnit)));
		} else {
	...
	protected function reduce($value, $forExpression = false) {
		...
		case "interpolate":
			...
			if (empty($value[2])) $res = $this->lib_echo($res);
		...
		case "variable":
			...
			if (is_array($key)) {
				$key = $this->reduce($key);
				$key = $this->vPrefix . $this->compileValue($this->lib_echo($key));
			}
		...
		case "string":
			foreach ($value[2] as &$part) {
				if (is_array($part)) {
					...
					if ($strip) $part = $this->lib_echo($part);
				}
			}
			return $value;
		case "escape":
			list(,$inner) = $value;
			return $this->lib_echo($this->reduce($inner));
		case "function":
	...
	public function compileFile($fname, $outFname = null) {
		...
		$out = $this->compile( call_user_func_array( self::FILE_GET . 'contents', Array( $fname ) ), $fname);

		$this->importDir = $oldImport;

		if ($outFname !== null) {
			return call_user_func_array( self::FILE_PUT . 'contents', Array( $outFname, $out ) );
		}
...
class lessc_parser {
	...
	// seek to a bp in the buffer or return where we are on no argument
