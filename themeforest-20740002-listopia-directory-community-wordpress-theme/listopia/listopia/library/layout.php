...
class jvbpd_layout_function {
	...
	public function __construct() {
		...
		add_action( 'jvbpd_load_header_file_name', array( $this, 'load_header_file' ), 10, 1 );
	...
	public function initialize() {
		/* Sidebar */{
			...
			if( strtolower( jvbpd_tso()->themeName ) == 'listopia' ) {
	...
	public function body_class( $classes=Array() ) {
		...
		if( empty( $strNavPosition ) ) {
			$defaultType = $this->getThemeType() == 'jvd-lp' ? 'full' : 'fixed';
			$strNavPosition = jvbpd_tso()->header->get( 'header_fullwidth', $defaultType );
		}
	...
	public function load_header_file( $subname=null ) {
		if( 'jv-nav-row-2-bottom-search' === jvbpd_header()->options->header_type ) {
			$subname = 'bottom-search';
		}
		return $subname;
	}
	...
	public function getLogo( $args=Array() ) {
		...
		if( jvbpd_layout()->getThemeType() == 'jvd-lk' ){
			...
			$defaults = Array(
				'no_image' => JVBPD_IMG_DIR .'/no-image.png',
				'logo_url' => JVBPD_IMG_DIR .'/listopia-white.png',
				'logo_light_url' => JVBPD_IMG_DIR .'/listopia-white.png',
				'logo_small_url' => JVBPD_IMG_DIR .'/jv-logo4.png',
			);
		 }
		...
		if( !empty( $options[ 'allow_types' ] ) && is_array( $options[ 'allow_types' ] ) ) {
			if( !in_array( $strHeaderType, $options[ 'allow_types' ] ) ) {
				return sprintf( '<img alt="%s">', esc_html__( "Logo", 'listopia' ) );
			}
		}
	...
	public function getBannerImage() {
		...
		return sprintf( '<img src="%1$s" class="%2$s" alt="%3$s">', $StrBannerImageUrl, join( ' ', $classes ), esc_attr__( "Banner", 'listopia' ) );
	}
	...
	public function getBreadcrumb() {
		$breadcrumb_args = Array(
			'home' => sprintf(
				'<li class=""><a href="%1$s" title="%2$s" target="_self">%3$s</a></li>',
				esc_url(home_url( '/' )),
				esc_attr__( "Home", 'listopia' ),
				esc_html__( "Home", 'listopia' )
			),
		);
	...
	public function common_script() {
		$mail_alert_msg = Array(
			'to_null_msg'			=> esc_html__('Please, type email address.', 'listopia' )
			, 'from_null_msg'		=> esc_html__('Please, type your email adress.', 'listopia' )
			, 'subject_null_msg'	=> esc_html__('Please, type your name.', 'listopia' )
			, 'content_null_msg'	=> esc_html__('Please, type your message', 'listopia' )
			, 'failMsg'				=> esc_html__('Sorry, failed to send your message', 'listopia' )
			, 'successMsg'			=> esc_html__('Successfully sent!', 'listopia' )
			, 'confirmMsg'			=> esc_html__('Do you want to send this email ?', 'listopia' )
		);
		$jvbpd_favorite_alerts = Array(
			"nologin"				=> esc_html__('If you want to add it to your favorite, please login.', 'listopia' )
			, "save"				=> esc_html__('Saved', 'listopia' )
			, "unsave"				=> esc_html__('Unsaved', 'listopia' )
			, "error"				=> esc_html__('Sorry, server error.', 'listopia' )
			, "fail"				=> esc_html__('favorite register fail.', 'listopia' )
		); ?>
		<?php
	}
	...
	public function single_author_information() {
		$this->section_title(
			esc_html__( "About the author", 'listopia' ),
			'author'
		);
	...
	public function relative_posts() {
		...
		$this->section_title(
			esc_html__( "Related Posts", 'listopia' ),
			'relative-posts'
		);
	...
	public function javo_custom_navi( $output, $item=Array(), $depth=0, $args=Array() ) {
		...
		if( isset( $item->jv_menu ) ) {
			ob_start();
			$this->load_template( 'parts/part-menu-'. $item->jv_menu, Array( 'item' => $item ) );
			$output = ob_get_clean();
		}
		return $output;
	}
	...
	public function header_skin_option( $option='' ) {
		$option = jvbpd_header()->header_skin;
		if( empty( $option ) ) {
	...
	public function header_bg_option( $option='' ) {
		if( is_singular( 'post' ) ) {
			if( jvbpd_layout()->getThemeType() == 'jvd-lk' )
				...
				$option = jvbpd_tso()->header->get( 'post_header_bg', '#506ac5' );
		}
		return $option;
	}
	...
	public function getPageLayoutTypes() {
		return Array(
			'wide-0' => esc_html__( "Wide( Width : 1140px )", 'listopia' ),
			'wide-1' => esc_html__( "Wide( Width : 1280px )", 'listopia' ),
			'wide-2' => esc_html__( "Wide( Width : 1400px )", 'listopia' ),
		);
	}
}
